...
class BaseTrainer(object):
    ...
    def __init__(self, compute_model, config, logger):
        ...
        self.feed_dict_buffer = collections.deque(maxlen=30)
        self.logger = logger
        ...
        self.logdir = "{}{}/".format(config.machine.log_dir, self.ts)
        self.save_path = "{}{}/".format(config.machine.save_dir, self.ts)
    ...
    def get_feed_dict(self, next_batch, tensor_dict, mode='train'):
        ...
        if mode != 'train':
            self.feed_dict_buffer.append(feed_dict)
        return feed_dict
    ...
    def run_one_epoch(self, epoch, batch_init, next_batch, train_ops, is_training=True):
        ...
        try:
            while True:
                ...
                try:
                    if is_training:
                        _, loss_train, summary = self.session.run(train_ops, feed_dict=feed_dict)
                        _train_loss += loss_train
                    else:
                        loss_test, summary_val = self.session.run(train_ops[1:], feed_dict=feed_dict)
                        _val_loss += loss_test
                except tf.errors.InvalidArgumentError as e:
                    self.logger.warn("Pbbly met some issue on matrix not invertible, skip this example, skip one epoch")
                    break
    ...
    def prepare_train(self, data_generator, restore_graph_fns):
        ...
        train_ops = [self.compute_model.optimizer, self.compute_model.loss, summary_op]
